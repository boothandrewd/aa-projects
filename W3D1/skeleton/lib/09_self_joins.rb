...
def haymarket_and_leith
  ...
  execute(<<-SQL)
    SELECT DISTINCT
      routesa.company, routesa.num
    FROM
      stops stopa
    JOIN
      routes routesa ON (routesa.stop_id = stopa.id)
    JOIN
      routes routesb ON (routesa.num = routesb.num AND routesa.stop_id != routesb.stop_id)
    JOIN
      stops stopb ON (routesb.stop_id = stopb.id)
    WHERE
