...
def sparse_continents
  ...
  execute(<<-SQL)
    ...
    -- Thanks to Nick Cannon for this one
    -- Subquery gets all countries with populations > 25,000,000, but only
    --  returns a list of their continents. This means any continent in the
    --  returned list has at least one country with a population greater than
    --  the alotted amount, making it an invalid continent
    WHERE
      continent NOT IN (
        SELECT
          continent
