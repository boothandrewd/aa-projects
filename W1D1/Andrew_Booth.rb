...
def base2to10(binary)
	...
	# your code goes here
  binary.chars.reverse.each_with_index.map do |char, index|
    char.to_i * 2**index
  end.reduce(:+)
end
...
class Hash
	...
	def my_select(&prc)
		...
		# your code goes here
    each_with_object({}) do |key_value, new_hash|
      key, value = key_value
...
class Hash
	...
	def my_merge(hash, &prc)
		...
		# your code goes here
    hash.each_with_object(dup) do |new_key_value, new_hash|
      key, new_value = new_key_value
      new_hash[key] = if prc.nil? || self[key].nil?
        new_value
      else
        prc.call(key, self[key], new_value)
      end
    end
	end
...
def lucas_numbers(n)
	...
	# your code goes here
  (0...n.abs).each_with_object([-1, 2]) do |_, nums|
    nums << nums.first + nums.last
    nums.shift
  end.last * ((n > -1 || n.even?) ? 1 : -1)
end
...
def longest_palindrome(string)
	...
	# your code goes here
  palindromes = (0...string.length).reduce([]) do |outer, l_idx|
    outer + ((l_idx + 2)...string.length).reduce([]) do |inner, r_idx|
      inner << string[l_idx..r_idx]
    end
  end.uniq.select { |word| word == word.reverse }.sort_by(&:length)

  palindromes.empty? ? false : palindromes.last.length
end
